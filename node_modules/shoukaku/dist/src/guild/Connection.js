...
class Connection extends events_1.EventEmitter {
    ...
    async connect(options) {
        ...
        try {
            const [status] = await (0, events_1.once)(this, 'connectionUpdate', { signal: controller.signal });
            if (status !== Constants_1.VoiceState.SESSION_READY) {
                if (status === Constants_1.VoiceState.SESSION_ID_MISSING)
    ...
    setServerUpdate(data) {
        if (!data.endpoint) {
            this.emit('connectionUpdate', Constants_1.VoiceState.SESSION_ENDPOINT_MISSING);
            return;
        }
        if (!this.sessionId) {
            this.emit('connectionUpdate', Constants_1.VoiceState.SESSION_ID_MISSING);
            return;
        }
        ...
        this.player.node.queue.add({ op: Constants_1.OPCodes.VOICE_UPDATE, guildId: this.guildId, sessionId: this.sessionId, event: this.serverUpdate });
        ...
        this.emit('connectionUpdate', Constants_1.VoiceState.SESSION_READY);
    }
    /**
    ...
    destroyLavalinkPlayer() {
        this.player.node.queue.add({ op: Constants_1.OPCodes.DESTROY, guildId: this.guildId });
